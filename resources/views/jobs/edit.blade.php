<x-layout>
    ...
    <div class="bg-white mx-auto p-8 rounded-lg shadow-md w-full md:max-w-3xl">
        ...
        <form method="POST" action="{{ route('jobs.update', $job->id) }}" enctype="multipart/form-data">
            ...
            <x-inputs.textarea name="description" id="description" placeholder="Enter Job Description"
                label="Job Description" :value="old('description', $job->description)" />

            <x-inputs.text id="salary" name="salary" label="Salary" type="number" placeholder="90000"
                :value="old('salary', $job->salary)" />

            <x-inputs.textarea name="requirements" id="requirements" placeholder="Enter Requirements"
                label="Requirements" :value="old('requirements', $job->requirements)" />
            ...
            <x-inputs.textarea name="company_description" id="company_description"
                placeholder="Enter Company Description" label="Company Description" :value="old('company_description', $job->company_description)" />
            ...
            <x-inputs.text id="contact_phone" name="contact_phone" label="Contact Phone"
                placeholder="Enter Company Phone" label="Company Phone" :value="old('contact_phone', $job->contact_phone)" />

            <x-inputs.text id="contact_email" name="contact_email" label="Contact Email" type="email"
                placeholder="Enter Company Email" :value="old('contact_email', $job->contact_email)" />
